...
<!--        <td class='editabledd warning'>{{$d->bookmakerName}}</td>-->
    ...
    <tr class="{{$d->match_id}}" id="{{isset($d->games_id)?$d->games_id.'!':''}}">
        ...
        <td>{{$d->income}}</td>
        <td>{{round(($d->income - $d->bsf - $d->bet), 2, PHP_ROUND_HALF_UP)}}</td>
        <td>@if($d->resultShort == '-')  <a role="button" @if ($countpl[$d->id] != 0) class="btn btn-default btn-xs" @else class="btn btn-primary btn-xs" @endif style="width: 50px" href="/confirm/{{$d->games_id}}/{{$d->game_type_id}}/true" style="font-size: 130%;">+&nbsp({{ (array_key_exists($d->match_id, $countpl))?$countpl[$d->match_id]:$countpl[$d->id] }})<span style='display: none;'>{{$d->match_id}}</span></a>
            @else
